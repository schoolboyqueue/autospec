...
- missing required field: tasks.branch
- invalid dependency reference in task T005: T999 does not exist
- invalid enum value for phases[0].tasks[0].status: expected one of [Pending, InProgress, Completed]
...
| `invalid dependency reference: T999 does not exist` | Task depends on non-existent task | Fix dependency ID or remove it |
| `tasks.branch does not match pattern` | Branch name format wrong | Use `NNN-name` format |
| `duplicate task ID: T001` | Same ID used twice | Ensure unique task IDs |
...
2. Fix task T005's dependencies to reference only existing task IDs
3. Change the invalid status value to one of: `Pending`, `InProgress`, or `Completed`
