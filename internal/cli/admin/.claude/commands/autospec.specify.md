...
4. **Preserve user intent**: The original user input follows the blank line after errors
...
| `feature.branch does not match pattern` | Branch name format wrong | Use `NNN-name` format |
...
2. Ensure `feature.branch` is included in the output
3. Process "Create a user authentication feature" as the feature description
